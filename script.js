        ...
        .then(users => {
            if (users.length > 0) {
                ...
                .catch(error => {
                    ...
                    document.getElementById("signupMessage").innerText = "An error occurred during signup. Please try again later.";
                });
            }
        })
        .catch(error => {
    ...
    fetch(`https://findkeja-server.onrender.com/users?username=${loginUsername}&password=${loginPassword}`)
        ...
        .then(users => {
            if (users.length > 0) {
                ...
                document.getElementsByClassName("container")[0].style.display = "none"; // Hide the login container
            } else {
                // User not found, display error message
                document.getElementById("loginMessage").innerText = "User not found. Please signup.";
            }
        })
        .catch(error => {
